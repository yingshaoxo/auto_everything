...
2D打印机 -> 经过升级 -> 3D打印机
3D打印机 -> 可以拿来生产无数新的打印机 -> 新的打印机有压制方块和雕刻方块的功能 -> 自主生产一切

3D打印机 -> 经过升级 -> 能打印电脑组件 -> 可以组成一台自由新计算机 -> 自主生产更高端的事物

```
电磁留声机 + 2D打印机 = 存储器;

升级(2D打印机) = 3D打印机;
不断打印它自己的零件(3D打印机) = 生产出无数新打印机; #压制,雕刻
打印万物(无数新打印机) = 自主生产一切;

升级(3D打印机) = 能打印电脑组件;
组装(电脑组件) = 自由新计算机;
使用(自由新计算机) = 自主生产更高端的一切;

#我发现python等编程语言，等号前面是结果，但在数学概念里，等号后面才是结果。
```

...
class VNCScreenshotClient:
    ...
    def send_key_event(self, down, key):
        ...
        if keyboard_key_code_dict == None:
            # Convert key to keycode (simplified)
            if isinstance(key, str) and len(key) == 1:
                key_code = ord(key)
            else:
                # Use some common key mappings
                key_mappings = {
                    'enter': 65293,
                    'return': 65293,
                    'space': 32,
                ...
                key_code = key_mappings.get(key.lower(), 0)
        else:
